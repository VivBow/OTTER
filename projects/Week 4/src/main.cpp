...
std::string windowTitle = "NatashaBowles_100782680";
...
int main() {
	...
	//VBO - Vertex buffer object
	VertexBuffer::Sptr cornerVbo = VertexBuffer::Create();
	cornerVbo->LoadData(corners, 9);

	VertexBuffer::Sptr pretty_vbo = VertexBuffer::Create();
	pretty_vbo->LoadData(pretty, 9);

	VertexArrayObject::Sptr vaoTree = VertexArrayObject::Create();
	vaoTree->AddVertexBuffer(cornerVbo, {
		BufferAttribute(0, 3, AttributeType::Float, 0, NULL)
		});
	vaoTree->AddVertexBuffer(pretty_vbo, {
		{ 1, 3, AttributeType::Float, 0, NULL }
	});
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transformTree = glm::translate(glm::mat4(1.0f), glm::vec3(0, glm::sin(static_cast<float>(thisFrame)), 0.f));
		...
		shader -> SetUniformMatrix("u_ModelViewProjection", camera -> GetViewProjection()* transformTree);
